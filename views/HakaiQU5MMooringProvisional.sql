	...
	FROM
		(
			SELECT
				...
				d0.*
			FROM
				sn.qu5_mooring_0_3m_5minute d0
			UNION
			ALL
			SELECT
				2 AS depth,
			...
			SELECT
				...
				d7.*
			FROM
				sn.qu5_mooring_7_5m_5minute d7
			UNION
			ALL
			SELECT
				10 AS depth,
				d10.*
			FROM
				sn.qu5_mooring_10m_5minute d10
			UNION
			ALL
			SELECT
				15 AS depth,
				d15.*
			FROM
				sn.qu5_mooring_15m_5minute d15
			UNION
			ALL
			SELECT
				20 AS depth,
				d20.*
			FROM
				sn.qu5_mooring_20m_5minute d20
			UNION
			ALL
			SELECT
				25 AS depth,
				d25.*
			FROM
				sn.qu5_mooring_25m_5minute d25
			UNION
			ALL
			SELECT
				30 AS depth,
				d30.*
			FROM
				sn.qu5_mooring_30m_5minute d30
			UNION
			ALL
			SELECT
				40 AS depth,
				d40.*
			FROM
				sn.qu5_mooring_10m_5minute d40
			UNION
			ALL
			SELECT
				50 AS depth,
				d50.*
			FROM
				sn.qu5_mooring_50m_5minute d50
			UNION
			ALL
			SELECT
				60 AS depth,
				d60.*
			FROM
				sn.qu5_mooring_60m_5minute d60
		) water FULL
		OUTER JOIN (
			SELECT
				-0.5 AS depth,
